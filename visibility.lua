...
function Visibility.ShouldShow(config)
    ...
    if instance then
        local ok = (instance.showInOpenWorld and cache.instanceType == "none")
            or (instance.showInParty and cache.instanceType == "party")
            or (instance.showInRaid and cache.instanceType == "raid")
            or (instance.showInArena and cache.instanceType == "arena")
            or (instance.showInPvp and cache.instanceType == "pvp")
            or (instance.showInScenario and cache.instanceType == "scenario")
        if not ok then return false end
    end

    local role = config.role
    if role then
        local ok = (role.showTank and cache.role == "TANK")
            or (role.showHealer and cache.role == "HEALER")
            or (role.showDps and (cache.role == "DAMAGER" or cache.role == "NONE"))
        if not ok then return false end
    end
