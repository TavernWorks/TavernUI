...
local function GetCastSpellIDForBuff(buffSpellID)
    local db = GetDB()
    if not db or not db.spells then return nil end
...
local function ScanSpellFromBuffs(castSpellID, itemID)
    ...
    if GetScannedSpell(castSpellID) then return true end
...
local function ProcessPendingScanning()
    ...
    if not next(module.pendingScanning) then return end
...
function module:SetAutoScan(enabled)
    ...
    local db = GetDB()
    if db then db.autoScan = enabled end
end
...
function module:RemoveSpell(spellID)
    ...
    if not spellID then return false end
    local db = GetDB()
    if not db or not db.spells or not db.spells[spellID] then return false end
...
function module:RemoveItem(itemID)
    ...
    if not itemID then return false end
    local db = GetDB()
    if not db or not db.items or not db.items[itemID] then return false end
    db.items[itemID] = nil
    return true
end
...
function module:BuildOptions()
    ...
    local args = {
        ...
        scanMode = {
            ...
            order = 10,
            get = function() return module.scanMode end,
    ...
    args.addSpellId = {
        ...
        order = 101,
        get = function() return module._addSpellId end,
        set = function(_, value) module._addSpellId = value or "" end,
    }
    args.addSpellDuration = {
        ...
        order = 102,
        get = function() return module._addSpellDuration end,
        set = function(_, value) module._addSpellDuration = value or "" end,
    }
    args.addSpell = {
        type = "execute",
        name = "Add spell",
    ...
    args.addItemId = {
        ...
        order = 104,
        get = function() return module._addItemId end,
        set = function(_, value) module._addItemId = value or "" end,
    }
    args.addItemDuration = {
        ...
        desc = "Buff duration in seconds.",
        order = 105,
        get = function() return module._addItemDuration end,
        set = function(_, value) module._addItemDuration = value or "" end,
    }
    args.addItem = {
        type = "execute",
        name = "Add item",
