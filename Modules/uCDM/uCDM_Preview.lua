...
local CooldownTracker = module.CooldownTracker
...
function Preview.ApplyPreviewFakeData(viewer, visibleItems)
    if not CooldownTracker or not CooldownTracker.CreateCooldownDuration then return end
    ...
    for _, entry in ipairs(visibleItems) do
        ...
        if frame then
            ...
            if cooldown then
                ...
                local durationObj = CooldownTracker.CreateCooldownDuration(startTime, PREVIEW_COOLDOWN_DURATION)
    ...
    local function scheduleNext()
        ...
        for _, frame in ipairs(viewer.__ucdmPreviewActiveFrames) do
            if frame and frame:IsShown() then
                local cooldown = frame.Cooldown or frame.cooldown
                if cooldown and CooldownTracker.CreateCooldownDuration and cooldown.SetCooldownFromDurationObject then
                    local durationObj = CooldownTracker.CreateCooldownDuration(now, PREVIEW_COOLDOWN_DURATION)
                    if durationObj then
                        cooldown:SetCooldownFromDurationObject(durationObj, true)
                        cooldown:Show()
                    end
